...
A [Pi](https://www.npmjs.com/package/@earendil-works/pi-coding-agent) extension that adds a `/provider` command for the custom providers in `~/.pi/agent/models.json` — relays, proxies and local OpenAI-compatible servers. Add one, pick its models, test it and switch to it without hand-editing JSON.

## What it looks like

**Provider list** (`/provider`) — one row per provider with its last test result; Enter opens it, `t` tests it:
...
**Test panel** — chat tests go through pi's own request path; a relay error that points at a compat flag comes with the fix (`c`), and Enter switches to a passing model:
...
**Provider page** (`/provider relay-one`) — every setting is a row you edit in place:
...
To work on it from a clone, see [Development](#development).
...
├── scripts/verify-pack.mjs    # checks what `npm publish` would ship
└── lib/
...
Run the tests with `npm test` (Node 22.19+). They cover the components (checklist, row menu, input, test panel), the wizard step machine, model diff/merge invariants, compat hints, and whole flows in TUI mode with scripted keys (add → review → save → test → switch, compat fix and re-test, key edit on the provider page) and in RPC mode.
